...
export const getValidatorStats = async (req, res) => {
  const managerId = req.user.managerAccount?.id;
  const userId = req.user.id; // Fallback if seller is scanning

  if (!managerId && req.user.role !== 'SELLER') {
  ...
  // Count validations by this person today
  const todayCount = await prisma.ticketValidation.count({
    where: {
      OR: [
        { validatedBy: managerId },
        { validatedByUser: userId }
      ],
  ...
  const totalCount = await prisma.ticketValidation.count({
    where: {
      OR: [
        { validatedBy: managerId },
        { validatedByUser: userId }
...
/**
 * For Offline Mode: Get basic ticket data to cache locally on phone
 * Only returns active tickets for campaigns owned by this manager's seller.
 */
export const getOfflineData = async (req, res) => {
  const managerId = req.user.managerAccount?.id;
  if (!managerId) throw new AuthorizationError('Manager access required');

  // 1. Get Manager's Seller ID
  const manager = await prisma.manager.findUnique({
    where: { id: managerId },
    select: { sellerId: true }
  });

  // 2. Find Active Campaigns (e.g., Event is Today or Tomorrow)
  ...
  const campaigns = await prisma.ticketCampaign.findMany({
    ...
    select: { id: true, title: true, isMultiScan: true, maxScansPerTicket: true }
  });

  const campaignIds = campaigns.map(c => c.id);

  // 3. Fetch Valid Tickets (Lightweight payload)
  const tickets = await prisma.ticket.findMany({
    where: {
      campaignId: { in: campaignIds },
      status: { in: ['VALID', 'USED'] } // Include USED so offline app knows they are used
    },
    select: {
      id: true,
      ticketNumber: true,
      qrCode: true, // Needed to match scan
      status: true,
      scanCount: true,
      maxScans: true,
      campaignId: true
...
/**
 * Sync Offline Scans back to Server
 */
export const syncOfflineValidations = async (req, res) => {
  const { offlineValidations } = req.body;
  const managerId = req.user.managerAccount?.id;
  ...
  if (!managerId) throw new AuthorizationError('Manager access required');
  if (!Array.isArray(offlineValidations)) throw new ValidationError('Invalid data format');

  const result = await validationService.processOfflineValidations(offlineValidations, managerId);
