...
class ValidationAnalyticsService {
  /**
   * Get Live Event Stats (Simple Dashboard)
   */
  async getCampaignLiveStats(campaignId) {
    ...
    // 2. Total Scans (Entries)
    const activeScans = await prisma.ticketValidation.count({
      where: { campaignId, isValid: true }
    });

    // 3. Unique People Inside
    // (If mult-scan is disabled, this = activeScans. If enabled, use unique ticketIds)
    const uniqueTicketsScanned = await prisma.ticketValidation.findMany({
      where: { campaignId, isValid: true },
      distinct: ['ticketId'],
      select: { id: true }
    });
    ...
    // 4. Percentage
    const totalSold = campaign._count.tickets;
    const percentage = totalSold > 0 ? ((uniqueTicketsScanned.length / totalSold) * 100).toFixed(1) : 0;

    return {
      campaign: campaign.title,
      totalTickets: totalSold,
      totalScans: activeScans,
      peopleCheckedIn: uniqueTicketsScanned.length,
