...
/**
  ...
  async validateTicketQR(qrData, validatorInfo, context = {}) {
    ...
    // This confirms the QR was generated by OUR server and hasn't been tampered with.
    const verification = verifyQRData(qrData);
    
    if (!verification.valid) {
      // Return 400 immediately for bad crypto
      throw new ValidationError(verification.error || 'Invalid QR Code');
    }
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (!ticket) throw new NotFoundError('Ticket record not found in database');

      // B. STRICT SECURITY CHECK: Version Match
      // Does the scanned QR string match the latest one stored in the DB?
      // This invalidates old PDFs if the ticket was re-issued/regenerated.
      if (ticket.qrCode !== qrData) {
        throw new ConflictError('This ticket has been revoked or re-issued. Please use the latest ticket.');
      }

      // C. Campaign & Event Logic
      if (ticket.campaign.status !== 'ACTIVE') {
        throw new ValidationError(`Event is not active (Status: ${ticket.campaign.status})`);
      }

      // Check Authorization
      await this._verifyValidatorAuthorization(validatorInfo, ticket.campaign.sellerId, tx);

      // D. Ticket Status Logic
      ...
      const maxScans = ticket.maxScans;

      if (ticket.status === 'USED' || currentScans >= maxScans) {
        throw new ConflictError(`Already Used: ${currentScans}/${maxScans} scans`);
      }
      ...
      const validation = await tx.ticketValidation.create({
        data: {
          ...
          validatedBy: managerId || null,
          validatedByUser: userId || null,
          validationMethod: context.method || 'QR_SCAN',
      ...
      return {
        valid: true,
        message: newStatus === 'USED' ? 'Entry Approved (Final Scan)' : `Entry Approved (${newScanCount}/${maxScans})`,
        ticket: {
          id: updatedTicket.id,
          ticketNumber: updatedTicket.ticketNumber,
          status: updatedTicket.status,
          scans: `${newScanCount}/${maxScans}`,
          holder: `${ticket.booking.customer.firstName} ${ticket.booking.customer.lastName}`
        },
        campaign: {
          title: ticket.campaign.title,
          venue: ticket.campaign.venue
        },
